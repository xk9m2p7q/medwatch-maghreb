  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 150,
        messages: [
          { role: 'system', content: `Translate the following news headline to ${langName}. Return only the translation, nothing else.` },
  ...
  for (const result of allResults) {
    ...
    try {
      for (const article of articles.slice(0, 5)) {
        ...
        // Always ensure English title; translate non-English sources
        const titleEn = isEnglishOrSpanish(rawTitle) ? rawTitle : await translateTitle(rawTitle, 'en');
        // Always generate Spanish translation
        const titleEs = await translateTitle(titleEn, 'es');
